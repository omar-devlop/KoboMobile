...
class UsersCubit extends Cubit<UsersState> {
  UsersCubit() : super(const UsersState.loading(msg: '')) {
    getSavedAccounts();
  }
  ...
  getSavedAccounts() async {
    ...
    if (accountsList.isEmpty) {
      safeEmit(const UsersState.empty());
    } else {
  ...
  Future<bool> savedAccountLogin(Account account) async {
    safeEmit(
      UsersState.savedUsers(
        data: accountsList,
        loggingUserName: account.username,
      ),
    );
    ...
    if (isAuth is String || (isAuth is bool && !isAuth)) {
      ...
      safeEmit(
        UsersState.loginError(
          data: accountsList,
          loggingUserName: account.username,
          error: isAuth.toString(),
        ),
      );
      return false;
    }
